...
from sklearn.tree import DecisionTreeRegressor # <- تغییر کلیدی: وارد کردن درخت تصمیم
...
def process_record(base_file_name, folder_path):
    ...
    try:
        ...
        correlated_ptt = []; correlated_hr = []; correlated_sbp = []; correlated_dbp = []; correlated_event_samples = []
        for i in range(1, len(ecg_peaks)):
            r_peak = ecg_peaks[i]
            rr_interval_sec = (r_peak - ecg_peaks[i-1]) / fs; heart_rate = 60.0 / rr_interval_sec
        ...
        if len(correlated_ptt) > 1:
            ptt_array = np.array(correlated_ptt); hr_array = np.array(correlated_hr)
            ptt_mean, ptt_std = np.mean(ptt_array), np.std(ptt_array); hr_mean, hr_std = np.mean(hr_array), np.std(hr_array)
            if ptt_std == 0: ptt_std = 1; 
            if hr_std == 0: hr_std = 1
            ptt_normalized = (ptt_array - ptt_mean) / ptt_std; hr_normalized = (hr_array - hr_mean) / hr_std
            print(f"Successfully processed and normalized. Found {len(ptt_normalized)} data points.")
            return ptt_normalized, hr_normalized, correlated_sbp, correlated_dbp, abp_signal_raw, correlated_event_samples, fs
        else:
            print("Not enough data to normalize. Skipping record."); return [], [], [], [], None, [], None
    except Exception as e:
        print(f"Could not process record {base_file_name}. Error: {e}"); return [], [], [], [], None, [], None
...
if __name__ == "__main__":
    ...
    print(f"\nTotal data points for training: {len(all_ptt_norm)}")
    
    X_train = np.column_stack((all_ptt_norm, all_hr_norm))
    y_sbp_train = np.array(all_sbp); y_dbp_train = np.array(all_dbp)
    
    print("Training the final Decision Tree models...")
    sbp_model = DecisionTreeRegressor(random_state=42)
    sbp_model.fit(X_train, y_sbp_train)
    
    dbp_model = DecisionTreeRegressor(random_state=42)
    ...
    if len(ptt_test_norm) > 0:
        ...
        plt.plot(event_samples_test, sbp_predicted, '*', color='orange', markersize=10, label='Predicted SBP (Decision Tree)')
        plt.plot(event_samples_test, dbp_predicted, '*', color='cyan', markersize=10, label='Predicted DBP (Decision Tree)')
        plt.title('Final Model Evaluation with Decision Tree')
        plt.xlabel('Sample Number'); plt.ylabel('Blood Pressure (mmHg)'); plt.legend(); plt.grid(True)
