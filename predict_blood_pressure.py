...
def extract_features_from_new_file(base_file_name, folder_path):
    ...
    try:
        ...
        return ptt_values_ms, hr_values, event_samples, ecg_signal_raw, ppg_signal_raw, fs
    except Exception as e:
        print(f"Could not process file {base_file_name}. Error: {e}"); return [], [], [], None, None, None

if __name__ == "__main__":
    print("Loading trained models and parameters...")
    sbp_model = joblib.load('sbp_model.joblib')
    dbp_model = joblib.load('dbp_model.joblib')
    norm_params = joblib.load('norm_params.joblib')
    print("Models loaded successfully.")
    folder_path = "D:/New folder (6)"
    
    new_file_name = "3042143_0013" 

    ptt_new, hr_new, samples_new, ecg_wave, ppg_wave, fs_new = extract_features_from_new_file(new_file_name, folder_path)
    if len(ptt_new) > 0:
        ...
        ax1.plot(ecg_wave, label='Input ECG Signal')
        ax1.set_title('Input Signals')
        ax1.legend()
        ax1.grid(True)
        ax2 = ax1.twinx() 
        ax2.plot(ppg_wave, label='Input PPG Signal', color='red', alpha=0.6)
        ax2.legend(loc='lower right')
        
        ax3 = plt.subplot(2, 1, 2, sharex=ax1)
        ax3.plot(samples_new, sbp_predicted, '*', color='red', markersize=8, label='Predicted SBP')
        ax3.plot(samples_new, dbp_predicted, '*', color='blue', markersize=8, label='Predicted DBP')
        ax3.set_title('Predicted Blood Pressure Envelope')
        ax3.set_xlabel('Sample Number')
        ax3.set_ylabel('Predicted Blood Pressure (mmHg)')
        ax3.legend()
        ax3.grid(True)
