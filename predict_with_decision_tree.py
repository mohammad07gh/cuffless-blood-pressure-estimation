...
if __name__ == "__main__":
    print("Loading trained Decision Tree models and parameters...")
    sbp_model = joblib.load('sbp_model_dt.joblib')
    dbp_model = joblib.load('dbp_model_dt.joblib')
    norm_params = joblib.load('norm_params_dt.joblib')
    ...
    if len(ptt_new) > 0:
        ...
        ax3.plot(samples_new, sbp_predicted, '*', color='orange', markersize=8, label='Predicted SBP (Decision Tree)')
        ax3.plot(samples_new, dbp_predicted, '*', color='cyan', markersize=8, label='Predicted DBP (Decision Tree)')
