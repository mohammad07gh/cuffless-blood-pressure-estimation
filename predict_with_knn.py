...
if __name__ == "__main__":
    print("Loading trained KNN models and parameters...")
    sbp_model = joblib.load('sbp_model_knn.joblib')
    dbp_model = joblib.load('dbp_model_knn.joblib')
    norm_params = joblib.load('norm_params_knn.joblib')
    ...
    if len(ptt_new) > 0:
        ...
        ax3.plot(samples_new, sbp_predicted, '*', color='green', markersize=8, label='Predicted SBP (KNN)')
        ax3.plot(samples_new, dbp_predicted, '*', color='purple', markersize=8, label='Predicted DBP (KNN)')
