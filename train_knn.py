...
from sklearn.neighbors import KNeighborsRegressor # <- تغییر کلیدی: وارد کردن KNN
import joblib
...
if __name__ == "__main__":
    ...
    sbp_model = KNeighborsRegressor(n_neighbors=7)
    sbp_model.fit(X_train, y_sbp_train)
    
    dbp_model = KNeighborsRegressor(n_neighbors=7)
    dbp_model.fit(X_train, y_dbp_train)
    print("Models trained successfully.")

    print("\nSaving models and normalization parameters...")
    joblib.dump(sbp_model, 'sbp_model_knn.joblib')
    joblib.dump(dbp_model, 'dbp_model_knn.joblib')
    norm_params = {'ptt_mean': ptt_mean, 'ptt_std': ptt_std, 'hr_mean': hr_mean, 'hr_std': hr_std}
    joblib.dump(norm_params, 'norm_params_knn.joblib')
