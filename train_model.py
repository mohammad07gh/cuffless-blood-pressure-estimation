...
def process_record(base_file_name, folder_path):
    ...
    try:
        ...
        print(f"Successfully processed. Found {len(correlated_ptt)} valid data points.")
...
for record_name in record_list:
    ptt, hr, sbp, dbp = process_record(record_name, folder_path)
    if len(ptt) > 0:
...
joblib.dump(sbp_model, 'sbp_model.joblib')
joblib.dump(dbp_model, 'dbp_model.joblib')
norm_params = {
    'ptt_mean': ptt_mean,
    'ptt_std': ptt_std,
    'hr_mean': hr_mean,
...
joblib.dump(norm_params, 'norm_params.joblib')
print("Files saved successfully: sbp_model.joblib, dbp_model.joblib, norm_params.joblib")